...
class SpiderDisesea:
    ...
    def get_items(self):
        try:
            #get main page
            self.browser.get("https://dxy.com/surgerys")

            #get aid_card
            soup = BeautifulSoup(self.browser.page_source)
            aid_cards = soup.find_all('a', attrs = {'class' : 'section-card common-card-link'})

            surgrys = []
            for card in aid_cards :
                name = card.find('div', attrs = {'class' : 'section-card-title'})
                desc = card.find('div', attrs = {'class' : 'section-card-desc'})
                href = card.attrs['href']

                item = {}
                item['name'] = name.get_text()
                item['brief'] = desc.get_text()
                item['url'] = href
                surgrys.append(item)
    ...
    def get_content(self, url):
        try:
            ...
            text = ''
            content = soup.find('div', attrs = {'class' : 'content-info'})
            text = ''
            ul = content.find('ul')
    ...
    def save(self, first_aid, content):
        try:
            headers = ['名称', '简介', '内容']
            file = path + '/丁香医生-查检查.csv'

            datas = [first_aid['name'], first_aid['brief'], content]
